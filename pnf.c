...
void activBox(){//{{{
	...
	sprinter(34, 7, DEFBG, 97, "Sorry, idk where i put this page ~^~ ");
	sprinter(45, 9, DEFBG, 97,            "ERROR: 404 ");
	sprinter(43,10, DEFBG, 97,          "Page not found");
	sprinter(ofX+2,11, DEFBG, 97, "                                       ────┬────    ");
	sprinter(ofX+2,12, DEFBG, 97, "   │      │      │       │      │          │        ");
	sprinter(ofX+2,13, DEFBG, 97, "   └┐    ┌┴┐    ┌┘       └┐    ┌┘          │        ");
	sprinter(ofX+2,14, DEFBG, 97, "    └────┘ └────┘         └────┘                    ");
...
void regularOutput(){
	...
	sprinter(16,1, 95, DEFBG, "/page-not-found");
	sprinter(WIDTH-20, 1, 37, DEFBG, "(minimum size: 80x24)");
	sprinter(32,1, 37, DEFBG,"World's most boring trannie\n");
	activBox();
	shoru();
	indexBox();	
	sprinter(11,20, 37, DEFBG,"You might want to go to one of these ->");
}
